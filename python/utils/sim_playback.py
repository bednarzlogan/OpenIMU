...
ground_truth_path = r"simulated_logs/ground_truth_2025_09_16_22_45_24.csv"
...
ax.plot(ground_truth["x"], ground_truth["y"], "k--", label="Path")
robot_marker, = ax.plot([], [], "ro", markersize=8, label="Robot")
trace_line, = ax.plot([], [], "b-", lw=1, label="Trace")
ax.legend()
ax.set_xlim(ground_truth["x"].min() - 1, ground_truth["x"].max() + 1)
ax.set_ylim(ground_truth["y"].min() - 1, ground_truth["y"].max() + 1)
ax.set_aspect("equal")
