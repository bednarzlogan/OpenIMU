...
TEST(SimScenario, Fifty_Meter_Loop) {
  ...
  P0.diagonal() << 0.5, 0.5, 0.5, 0.1, 0.1, 0.1, 0.25, 0.25, 0.25, 1e-4, 1e-4,
      1e-4, 1e-4, 1e-4, 1e-4;
  ...
  const auto period = std::chrono::milliseconds(
      10); // worker tick; sim uses file timestamps anyway
  ...
  // std::this_thread::sleep_for(2ms); if (!sim.finished()) {
  // sim.stop_simulation(); FAIL() << "scenario timed out"; }

  // Assert: filter reached end time and ended near expected endpoint (from
  // scenario config) Suppose your scenario doc (JSON) includes
  // "expected_final_enu": [E, N, U] and "pos_tol_m": 1.0 If you don't parse
  // JSON here, hardcode the expected for now:
  const Eigen::Vector3d expected_E_N_U(
      -0.11, 0.15, 0.0); // endpoint of the log for this test
  const double pos_tol =
      0.5; // TODO this should be set from the lower bound of the estimator

  const auto &xN = ukf->get_state(); // N-vector
  const Eigen::Vector3d pos =
      xN.template segment<3>(0); // assuming first 3 are position
  ...
  EXPECT_GE(ukf->solution_time(), 44.1 - 1e-6);
