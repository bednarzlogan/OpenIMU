...
inline std::string formatLogName(std::string base_name,
                                 std::string extension = ".txt") {
  auto time = std::time(nullptr);   // returns current time, null tells it we
                                    // aren't assigning to a timer
 ...
 * - Receiving IMU data (currently from a file, later possibly from a real-time
 * sensor).
 * - Performing a rolling average to smooth noisy IMU data.
 * - Delivering smoothed data to a consumer (e.g., a Kalman filter).
 *
 * Data ingestion and smoothing run in a dedicated thread. The class supports
 * configuration via JSON for queue length and smoothing parameters.
 */
...
public:
   ...
   *
   * @param path_to_configs Path to a JSON config file specifying smoothing
   * behavior.
   */
   ...
   *
   * This is a non-blocking call. It returns false if no smoothed measurements
   * are available.
   *
   ...
   *
   * @param smoothed_measurement Will be overwritten with a new measurement once
   * available.
   */
   ...
   *
   * This function reads and parses a CSV file of IMU data, pushes raw
   * measurements into the smoothing queue, and spawns a background thread to
   * smooth and prepare data.
   *
  ...
  void log_vector_out(Logger &diag_logger,
                      const Eigen::Matrix<double, Eigen::Dynamic, 1> &vec,
                      LoggedVectorType type) {
    switch (type) {
    case LoggedVectorType::NominalState:
      diag_logger.logMessage<N, float>(
          MSG_ID_NOMINAL_STATE, DataType::Float32, 0x01,
...
private:
  ...
  /** Internal buffer holding the most recent measurements for rolling
   * averaging. */
  ...
  /**
   * @brief The background loop that polls for measurements and triggers
   * smoothing.
   *
   * Runs in its own thread while `_reading == true`. Controlled via
   * `_startLoop` and `_stopLoop`.
   */
  void _loopTimer();
};
