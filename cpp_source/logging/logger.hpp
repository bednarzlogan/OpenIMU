...
enum class DataType : uint8_t {
  ...
  UInt16 = 0b01,  ///< 2-byte unsigned integer
  Float64 = 0b10, ///< 8-byte double (rare, high-precision)
  Int32 = 0b11    ///< 4-byte signed integer
};
 ...
 *
 * The structure is 9 bytes and includes timestamp, message ID, flags, and
 * payload info. No padding is inserted due to packing (verified by
 * static_assert).
 */
struct LogHeader {
...
public:
  ...
  Logger(const std::string &log_file_name = "diag_log.bin")
    ...
    std::strftime(buffer, sizeof(buffer),
                  "# Log Start Time: %Y-%m-%d %H:%M:%S UTC\n", tm);
    out_ << buffer;
  }
  ...
  void logMessage(uint16_t id, DataType type, uint8_t node_id,
                  const std::array<T, N> &payload) {
    // get timing info
    ...
    header.dlc = static_cast<uint8_t>(sizeof(T) * N);
...
private:
  ...
  std::chrono::steady_clock::time_point
      start_time_; ///< Time reference for timestamps
  std::chrono::system_clock::time_point
      wall_time_; ///< Real-world time for header
};
